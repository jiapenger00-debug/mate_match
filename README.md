...
| `/api/search` | POST | 执行搜索，返回 JSON 结果列表（前端异步展示，支持分别搜索男女双方） |
| `/api/share` | POST | 接收 JSON 分析结果，存入 SQLite，返回短 ID |
| `/s/<id>` | GET | 渲染分享名片页（精简卡片 + 可展开完整报告） |
| `/api/supplement` | POST | 接收补充信息（她/他各自文本+照片），结合原报告重新分析 |
...
- **index.html** — 表单页面，CSS Grid 双列布局；**标签式示例选择器**（4 组模板）；**双人异步搜索**（男女各自搜索，结果合并展示，支持跳过+超时自动降级）；**截图导入**（按钮+粘贴，上传前自动压缩）；**照片上传**（颜值评分）；**对对方的要求**字段；**明暗主题切换**
- **result.html** — 报告页面，通过 Jinja2 模板语法渲染；底部**分享栏**（复制链接 + toast 提示）+ **补充分析按钮**（弹窗补充信息后重新分析），支持生成公开分享页
