...
- 也可以手动填写：
  ...
  - **对对方的要求**：各自写出希望对方是什么样的（选填，帮助 LLM 判断期望是否匹配）
  - 年龄、家乡、职业、外貌特征（选填）
...
两个独立开关：「搜索女方的公开信息」和「搜索男方的公开信息」。默认都勾选，搜索时会同时查询双方的姓名 + 职业/家乡/学历组合关键词，结果合并展示在卡片中。

搜索超时（4 秒）自动跳过，也可随时点击「跳过，直接分析」按钮进入 AI 分析阶段。图片上传前会自动压缩（最大 1024px），手机端体验更流畅。

> ⚠️ 如果姓名是昵称或不方便搜索，建议取消勾选。
...
- 🌐 **网络参考信息** — 搜索到的公开资料（如有）
- 📋 **分享栏** — 复制链接发给朋友看（底部）
- 📝 **补充分析** — 点击「补充信息，进一步分析」弹窗补充她/他的信息或照片，AI 结合原报告重新分析
...
颜值匹配度 = `100 - |女方分 - 男方分| × 15`，差距越小分数越高：

| 匹配度 | 双方颜值差距 | 等级 | 含义 |
|--------|-------------|------|------|
| 92-100 | ≤ 0.5 分 | 💚 天作之合 | 颜值非常登对，看起来很配 |
| 78-91 | 0.5-1.5 分 | 💛 比较般配 | 颜值匹配度较高，在一起养眼 |
| 63-77 | 1.5-2.5 分 | 🧡 略有差距 | 颜值有一定差距，但互补也是魅力 |
| 0-62 | > 2.5 分 | ❤️ 差异较大 | 颜值差异较大，不过真爱不看脸 |
